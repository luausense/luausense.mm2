...
local function IsAdmin()
    return LocalPlayer.UserId == 1219983923
        or LocalPlayer.Name:lower() == "glebhleb32411"
                ...
                if hum and root and head and hum.Health > 0 then
                    ...
                    if not isSelf or Config.ESP.SelfESP then
                        ...
                        if onScreen and rootPos.Z >= 0 then
                            ...
                            local headPos = Camera:WorldToViewportPoint(head.Position + Vector3.new(0, 0.5, 0)); local legPos = Camera:WorldToViewportPoint(root.Position - Vector3.new(0, 3, 0))
                            local height = math.abs(headPos.Y - legPos.Y); if height < 10 then height = 10 end; local halfWidth = height / 4.44
                            if Config.ESP.Box then obj.Box.Visible = true; obj.Box.PointA = Vector2.new(rootPos.X - halfWidth, headPos.Y); obj.Box.PointB = Vector2.new(rootPos.X + halfWidth, headPos.Y); obj.Box.PointC = Vector2.new(rootPos.X + halfWidth, legPos.Y); obj.Box.PointD = Vector2.new(rootPos.X - halfWidth, legPos.Y); obj.Box.Color = color end
                            if Config.ESP.Name then obj.NameText.Visible = true; obj.NameText.Text = player.DisplayName; obj.NameText.Position = Vector2.new(rootPos.X, headPos.Y - 18); obj.NameText.Color = color end
                            obj.RoleText.Visible = true; obj.RoleText.Text = "[" .. role:upper() .. "]"; obj.RoleText.Position = Vector2.new(rootPos.X, headPos.Y - 32); obj.RoleText.Color = color
                            if Config.ESP.Distance then local dist = math.floor((Camera.CFrame.Position - root.Position).Magnitude); obj.DistanceText.Visible = true; obj.DistanceText.Text = "[" .. dist .. "m]"; obj.DistanceText.Position = Vector2.new(rootPos.X, legPos.Y + 4); obj.DistanceText.Color = color end
                            if Config.ESP.Tracer and not isSelf then obj.Tracer.Visible = true; obj.Tracer.From = GetTracerOrigin(Config.ESP.TracerOrigin); obj.Tracer.To = Vector2.new(rootPos.X, rootPos.Y); obj.Tracer.Color = color end
                            if Config.ESP.Skeleton then
                                for _, bone in ipairs(skeletonBones) do
                                    ...
                                    if p1 and p2 then
                                        ...
                                        local line = State.skeletonESP[player][idx]
                                        if line and vis1 and vis2 then line.Visible = true; line.From = Vector2.new(pos1.X, pos1.Y); line.To = Vector2.new(pos2.X, pos2.Y); line.Color = color end
                                    end
                                end
                            end
                        end
...
-- ОРИГИНАЛЬНЫЙ BULLET TP ИЗ ФАЙЛА
...
local function TeleportToPlayer(playerName)
    ...
    if target and target.Character then UpdateCache() if CachedRoot then local theirRoot = target.Character:FindFirstChild("HumanoidRootPart") if theirRoot then CachedRoot.CFrame = theirRoot.CFrame + Vector3.new(0, 0, 3) end end end
end
